...
def main(command = None , chat_id = None, dict_user_data = None,json_update = None):
    ...
    #4-> начало работы с делами
        ...
        #4->1->1 указываем дату
            ...
            reply_markup = {'keyboard': [['сегодня'],['меню']], 'resize_keyboard': True, 'one_time_keyboard': True}


        #4->1->1->1 записываем!
        elif state_info_1 == 'Записать дело' and state_info_3 is None:
            if command == 'сегодня':
                r = psql_methods.add_task(chat_id = chat_id  , date_plan = command , dict_user_data = dict_user_data)
                text = r['text']
